...
|-----------|-------------|
| `Detect_GenericOverload_LocalVariable` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` result is assigned to a local variable. |
| `Detect_GenericOverload_ReturnStatement` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` is returned directly. |
| `Detect_GenericOverload_MethodArgument` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` is passed as a method argument. |
| `Detect_GenericOverload_FieldInitializer` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` is used in a static field initializer. |
| `Detect_GenericOverload_LambdaBody` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` is called inside a lambda body. |
| `Detect_GenericOverload_TernaryExpression` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` appears in a ternary expression branch. |
| `Detect_GenericOverload_AsyncMethod` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` is called inside an async method. |
| `Detect_GenericOverload_RecordStruct` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationRecordStruct>()` is used with a [EnforceInitialization] record struct. |
| `Detect_GenericOverload_ReadonlyStruct` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationReadonlyStruct>()` is used with a [EnforceInitialization] readonly struct. |
...
| `NoReport_GenericOverload_EmptyEnforceInitializationStruct` | Ensure that SPIRE005 is NOT triggered when `CreateInstance<EmptyEnforceInitializationStruct>()` is used on a fieldless [EnforceInitialization] struct. |
...
| File Name | Description |
|-----------|-------------|
| `Detect_TypeOnly_LocalVariable` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is assigned to a local variable. |
| `Detect_TypeOnly_ReturnStatement` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is returned from a method. |
| `Detect_TypeOnly_MethodArgument` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is passed as a method argument. |
| `Detect_TypeOnly_LambdaBody` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is called inside a lambda body. |
| `Detect_TypeOnly_TernaryExpression` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` appears as a ternary branch. |
| `Detect_TypeOnly_RecordStruct` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationRecordStruct))` is used with a [EnforceInitialization] record struct. |
| `Detect_TypeOnly_ReadonlyStruct` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationReadonlyStruct))` is used with a [EnforceInitialization] readonly struct. |
| `Detect_TypeOnly_NestedInClass` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is called inside a nested class. |
...
| `NoReport_TypeOnly_EmptyEnforceInitializationStruct` | Ensure that SPIRE005 is NOT triggered when `CreateInstance(typeof(EmptyEnforceInitializationStruct))` is used on a fieldless [EnforceInitialization] struct. |
...
| File Name | Description |
|-----------|-------------|
| `Detect_NonPublic_TrueFlag` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct), nonPublic: true)` is called. |
| `Detect_NonPublic_FalseFlag` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct), nonPublic: false)` is called. |
| `Detect_NonPublic_RecordStruct` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationRecordStruct), true)` is used with a [EnforceInitialization] record struct. |
| `Detect_NonPublic_ReturnStatement` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct), true)` is returned from a method. |
| `Detect_NonPublic_LambdaBody` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct), false)` is called inside a lambda body. |
...
| `NoReport_NonPublic_EmptyEnforceInitializationStruct` | Ensure that SPIRE005 is NOT triggered when `CreateInstance(typeof(EmptyEnforceInitializationStruct), true)` is used on a fieldless [EnforceInitialization] struct. |
...
| `Detect_Params_RecordStruct_NullArgs` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationRecordStruct), (object[])null)` is used with a [EnforceInitialization] record struct. |
...
| `NoReport_Params_EmptyEnforceInitializationStruct_NullArgs` | Ensure that SPIRE005 is NOT triggered when `CreateInstance(typeof(EmptyEnforceInitializationStruct), (object[])null)` is used on a fieldless [EnforceInitialization] struct. |
...
| `Detect_ArgsActivation_RecordStruct` | Ensure that SPIRE005 IS triggered when used with a [EnforceInitialization] record struct and null args. |
...
| `NoReport_ArgsActivation_EmptyEnforceInitializationStruct` | Ensure that SPIRE005 is NOT triggered when the target type is a fieldless [EnforceInitialization] struct. |
...
| `Detect_BindingFlags_RecordStruct` | Ensure that SPIRE005 IS triggered when used with a [EnforceInitialization] record struct and null args. |
...
| `NoReport_BindingFlags_EmptyEnforceInitializationStruct` | Ensure that SPIRE005 is NOT triggered when the target type is a fieldless [EnforceInitialization] struct with null args. |
...
| `Detect_FullOverload_RecordStruct` | Ensure that SPIRE005 IS triggered when used with a [EnforceInitialization] record struct and null args. |
...
| `NoReport_FullOverload_EmptyEnforceInitializationStruct` | Ensure that SPIRE005 is NOT triggered when the target type is a fieldless [EnforceInitialization] struct with null args. |
...
## Category I: Not flagged — fieldless [EnforceInitialization] struct

### should_pass

| File Name | Description |
|-----------|-------------|
| `NoReport_EmptyEnforceInitialization_GenericOverload` | Ensure that SPIRE005 is NOT triggered for `CreateInstance<EmptyEnforceInitializationStruct>()` (fieldless, SPIRE002 handles this). |
| `NoReport_EmptyEnforceInitialization_TypeOnly` | Ensure that SPIRE005 is NOT triggered for `CreateInstance(typeof(EmptyEnforceInitializationStruct))`. |
| `NoReport_EmptyEnforceInitialization_NonPublic` | Ensure that SPIRE005 is NOT triggered for `CreateInstance(typeof(EmptyEnforceInitializationStruct), true)`. |
| `NoReport_EmptyEnforceInitialization_ParamsNullArgs` | Ensure that SPIRE005 is NOT triggered for `CreateInstance(typeof(EmptyEnforceInitializationStruct), (object[])null)`. |
...
| File Name | Description |
|-----------|-------------|
| `Detect_GenericOverload_DiscardAssignment` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` result is discarded (`_ = Activator.CreateInstance<EnforceInitializationStruct>()`). |
| `Detect_GenericOverload_ForeachLoop` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` is called inside a foreach loop body. |
| `Detect_TypeOnly_WhileLoop` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is called inside a while loop body. |
| `Detect_GenericOverload_NullCoalescing` | Ensure that SPIRE005 IS triggered when `CreateInstance<EnforceInitializationStruct>()` appears in a null-coalescing expression. |
| `Detect_TypeOnly_SwitchExpressionArm` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` appears in a switch expression arm. |
| `Detect_TypeOnly_NestedInStaticClass` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct))` is called in a static method of a static class. |
| `Detect_Params_NullArgs_LambdaBody` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct), (object[])null)` is inside a lambda body. |
| `Detect_Params_EmptyArgs_TernaryExpression` | Ensure that SPIRE005 IS triggered when `CreateInstance(typeof(EnforceInitializationStruct), new object[0])` is a ternary branch. |
