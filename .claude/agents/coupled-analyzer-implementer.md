...
Generator-coupled analyzers live in `src/Spire.Analyzers/SourceGenerators/Analyzers/` (not `src/Spire.Analyzers/Rules/`). They run on the output compilation after the generator has executed. They analyze generated code patterns specific to discriminated unions.
...
- Do NOT edit files outside `src/Spire.Analyzers/SourceGenerators/Analyzers/` and `src/Spire.Analyzers/SourceGenerators/AnalyzerDescriptors.cs`.
