...
3. Verify no existing descriptor matches `id: "$1"` in `src/Houtamelo.Spire.Analyzers/Descriptors.cs` — abort if duplicate

## Scaffolding (TDD order — tests and descriptor BEFORE analyzer)

4. Add descriptor to `src/Houtamelo.Spire.Analyzers/Descriptors.cs`
   - Field name: `{RuleId}_{TitlePascalCase}`
   - Add in sequential order
5. Create test folder: `tests/Houtamelo.Spire.Analyzers.Tests/{RuleId}/`
6. Create test case folder: `tests/Houtamelo.Spire.Analyzers.Tests/{RuleId}/cases/`
7. Create shared preamble: `tests/Houtamelo.Spire.Analyzers.Tests/{RuleId}/cases/_shared.cs`
   ...
   - Always include: `global using System;`, `global using System.Collections.Generic;`, `global using System.Threading.Tasks;`, `global using Houtamelo.Spire.Analyzers;`
...
8. Create test runner from template: `tests/Houtamelo.Spire.Analyzers.Tests/{RuleId}/{RuleId}Tests.cs`
